...
本 Fork 在上游基础上做了以下定制，便于作为个人导航页长期使用：

- 🔎 **可自定义搜索引擎**：默认提供 Google / Bing / Github；后台可新增、删除、排序和启用/停用站外搜索引擎，搜索 URL 使用 `{query}` 作为关键词占位符。
- 🔁 **Baidu 改为 Bing**：首页搜索标签、提示文字和实际搜索行为统一使用 Bing，同时兼容旧版本保存的 Baidu 选择。
- 🖼️ **书签 Logo 透明背景**：透明 PNG/SVG 不再被灰白底填充，并使用 `object-fit: contain` 避免图标被裁切。
- 🌐 **网站 Logo / Favicon 可配置**：后台可填写 HTTPS 图标地址，或直接上传 PNG/JPG/WebP/GIF/ICO 图片；可一键恢复默认图标。
- 🎨 **后台字体协调**：统一站点信息、网站图标、自定义搜索引擎等设置区域的标签、输入框、提示文字与按钮字号。
- 🧹 **缓存策略调整**：JavaScript 资源改为重新验证/不长期锁死，首页缓存版本随关键改动更新，避免部署后仍显示旧前端资源。
- 🔐 **私密分类祖先链保护**：匿名首页、分类列表、书签列表和单书签接口都要求“书签自身 + 所属分类 + 全部祖先分类”公开；私密祖先、孤儿分类、循环分类和异常脏数据默认 fail closed，批量操作和投稿审核也会继承完整分类链的隐私边界。
- 🔒 **关闭每日上游自动同步**：保留手动同步入口，避免上游定时同步覆盖本 Fork 的自定义修改。
...
<!-- changelog:start -->
- 🛡️ **2026-09-15**：加强私密数据保护：匿名读取按完整分类祖先链判断可见性，异常分类树与脏数据默认 fail closed，并同步收紧批量操作、投稿与审核路径。
- 🧩 **2026-09-15**：记录本 Fork 自定义功能：Bing/可配置搜索引擎、透明书签 Logo、自定义网站 Favicon、后台字体统一、缓存策略调整及关闭每日上游自动同步。
...
- 🎨️ **2025-12-18**：优化卡片样式与后台界面体验，并增强分类能力
